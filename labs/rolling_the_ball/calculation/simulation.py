...
class Calculator:
    ...
    def __call__(self, time_delta: float) -> Generator[Ball]:
        if not is_movement_without_slippage_possible(self.env):
            yield move_with_slippage(self.current_ball, self.env, time_delta)
        else:
            if is_slippage_finished(self.current_ball):
                yield move_without_slippage(self.current_ball, self.env, time_delta)
            else:
                yield move_with_slippage(self.current_ball, self.env, time_delta)
